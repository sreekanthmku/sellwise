...
export function CallNumberModal({ open, onOpenChange, initialLead, humanLeads }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn(
          "fixed bottom-0 left-1/2 top-auto z-50 flex max-h-[min(90vh,640px)] w-full max-w-[440px] -translate-x-1/2 translate-y-0 flex-col gap-0 overflow-hidden rounded-b-none rounded-t-[24px] border border-[#e9ebef] border-b-0 bg-white p-0 shadow-xl",
        )}
      >
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain px-5 pb-6 pt-5">
          ...
          <div className="mt-6 space-y-5">
            <div>
              <label
                ...
                className="font-body text-[14px] font-bold text-[color:var(--gray-300)]"
              >
                {m.phoneNumber}
              </label>
              <div className="mt-2 flex items-stretch overflow-hidden rounded-[14px] border border-[#e9ebef] bg-white">
                <span className="flex shrink-0 items-center gap-1.5 border-r border-[#e9ebef] bg-[#fafbfc] px-3 text-[15px] text-[color:var(--gray-200)]">
                ...
                <input
                  ...
                  className="min-w-0 flex-1 bg-transparent px-3 py-3 font-body text-[15px] font-medium text-[color:var(--gray-300)] outline-none placeholder:text-[color:var(--gray-200)]"
                />
              </div>
            </div>
                ...
                <Popover modal={false} open={attachOpen} onOpenChange={setAttachOpen}>
                  <PopoverTrigger asChild>
                    <button
                      ...
                      className={cn(
                        "flex w-full items-center justify-between gap-2 rounded-[14px] border bg-white py-3.5 pl-3 pr-3 text-left font-body text-[15px] font-medium text-[color:var(--gray-300)] outline-none transition-[border-color,box-shadow]",
                        attachOpen
                    ...
                    >
                      ...
                      <ChevronDown
                        className={cn(
                          "h-5 w-5 shrink-0 text-[color:var(--gray-200)] transition-transform duration-200",
                  ...
                  <PopoverContent
                    ...
                    collisionPadding={16}
                    className={cn(
                      "z-[100] rounded-[16px] border border-[#e5e8ee] bg-white p-1.5 shadow-[0_10px_40px_rgba(15,23,42,0.12)] outline-none",
                  ...
                  >
                    <ul role="listbox" aria-labelledby="call-modal-lead-label" className="flex flex-col gap-0.5">
                      {attachRows.map((row) => {
            ...
            <div>
              <p className="font-body text-[14px] font-bold text-[color:var(--gray-300)]">
                {m.saveContactAs}
              </p>
              <div className="mt-2 grid grid-cols-2 gap-3">
                {(["primary", "alternate"]).map((key) => (
                  <button
                    ...
                    className={cn(
                      "flex items-center justify-center gap-2 rounded-full border-2 py-3 font-body text-[14px] font-semibold transition-colors",
                      saveAs === key
            ...
            <div className="flex items-center gap-3 rounded-[14px] border border-[#eef0f3] bg-[#fafbfc] px-3 py-3.5">
              <Bell className="h-5 w-5 shrink-0 text-[color:var(--gray-200)]" strokeWidth={2} />
              <div className="min-w-0 flex-1">
                <p className="font-body text-[14px] font-bold text-[color:var(--gray-300)]">
                  {m.whatsappAvailable}
                </p>
                <p className="mt-0.5 font-body text-[12px] text-[color:var(--gray-200)]">
          ...
          <div className="mt-8 grid grid-cols-2 gap-3">
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-[14px] border-2 border-[color:var(--blue-600)] bg-white py-3.5 font-body text-[14px] font-semibold text-[color:var(--blue-600)] transition-colors hover:bg-[color:var(--blue-200)] disabled:pointer-events-none disabled:opacity-40"
            >
            ...
            <button
              ...
              onClick={goCall}
              disabled={!attachedLeadId}
              className="flex items-center justify-center gap-2 rounded-[14px] border-2 border-[color:var(--blue-600)] bg-[color:var(--blue-600)] py-3.5 font-body text-[14px] font-semibold text-white transition-colors hover:opacity-95 disabled:pointer-events-none disabled:opacity-40"
            >
